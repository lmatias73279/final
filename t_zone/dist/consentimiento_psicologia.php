...
$sql = "SELECT ID FROM consentimientos WHERE id_user = ? AND tipo_consentimiento = 1 AND estado = 0 LIMIT 1";
...
if (isset($_POST['submit'])) {
    ...
    if (!empty($firma_data)) {
        ...
        $sql = "UPDATE consentimientos SET firma = ?, fecha_sign = ?, estado = 1 WHERE ID = ? AND tipo_consentimiento = 1";
        $stmt = $conn->prepare($sql);
        ...
        if ($stmt->execute()) {
            ...
            $encryptedStatus = simpleEncrypt('ok_psicologo', '2020');
