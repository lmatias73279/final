...
$hoy = date('N'); // Día actual: 1 (lunes) a 7 (domingo)
$horaActual = date('H:i'); // Hora actual en formato HH:MM

// Variables por defecto
$fechaActual = null;
$horaCerrada = null;
$soloSiEsOchoAm = false;

switch ($hoy) {
    case 5: // Viernes
        if ($horaActual === '08:00') {
            $fechaActual = date('Y-m-d', strtotime('+3 days')); // Lunes
            $soloSiEsOchoAm = true;
        } else {
            exit(); // No hacer nada si no son las 08:00 AM
        }
        break;

    case 6: // Sábado
        if ($horaActual === '08:00') {
            $fechaActual = date('Y-m-d', strtotime('+3 days')); // Martes
            $soloSiEsOchoAm = true;
        } else {
            exit(); // No hacer nada si no son las 08:00 AM
        }
    ...
    default:
        // Lunes a jueves: calcular según la hora
        $fechaActual = date('Y-m-d', strtotime('+2 days'));
...
if ($soloSiEsOchoAm) {
    ...
    $sql = "SELECT fecha, hora, link_ingreso, psi, userID, tipo, `site`
            FROM sessions 
            WHERE fecha = ? AND hora = ? AND estado < 5";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ss", $fechaActual, $horaCerrada);
}
...
while ($row = $result->fetch_assoc()) {
    ...
    if($modalidad === 1){
      ...
      if($profesional_id === 26){
        $direccion = "Calle 2a # 72 - 76 Casa 107 - Agrupación vivienda Pio X, Barrio Americas Occidental";
      }else{
          $direccion = "Carrera 49a # 94 - 32 - Barrio La Castellana";
      }
    }else if($modalidad === 2){
      $modalidad = "Virtual";
    ...
    $payload = array(
      ...
      "internal_id" => "7e7ca375-cc07-4218-943f-7ac20e76ff7a",
