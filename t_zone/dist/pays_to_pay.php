                  ...
                  <div class="card-body">
                    <div class="table-responsive">
                      <table class="table table-bordered table-md">
                        ...
                        $sqlCount = "SELECT COUNT(*) AS total FROM sessions WHERE estado = 7";
                        ...
                        $sql = "SELECT * FROM sessions WHERE estado = 7 LIMIT $offset, $registrosPorPagina";
                        $result = $conn->query($sql);

                        // Inicializamos un contador para el número de fila
                        $count = $offset + 1;

                        // Array para almacenar los resultados agrupados por psi
                        $groupedResults = [];
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            // Si el psi ya está en el array, acumula los valores, si no, agrega una nueva entrada
                            if (isset($groupedResults[$psisql])) {
                                $groupedResults[$psisql]['valor'] += $row['valor'];
                                $groupedResults[$psisql]['ingresoRT'] += $row['ingresoRT'];
                            } else {
                                // Recuperar los datos de psi (psiquiatra)
                                $sqlpsi = "SELECT numdoc, pn_usu, pa_usu FROM usuarios WHERE id = $psisql";
                                $result1 = $conn->query($sqlpsi);
                                $row1 = $result1->fetch_assoc();

                                // Guardar la información del psi en el array
                                $groupedResults[$psisql] = [
                                    'numdoc' => $row1['numdoc'],
                                    'pn_usu' => $row1['pn_usu'],
                                    'pa_usu' => $row1['pa_usu'],
                                    'valor' => $row['valor'],
                        ...
                        // Ahora recorremos los resultados agrupados
                        $count = 1;
                        foreach ($groupedResults as $psisql => $data) {
                            echo '<tr>';
                            echo '<td style="text-align: center;">' . $count . '</td>';
                            echo '<td style="text-align: center;">' . $data['numdoc'] . '</td>';
                            echo '<td style="text-align: center;">' . $data['pn_usu'] . ' ' . $data['pa_usu'] . '</td>';
                            echo '<td style="text-align: center;">$ ' . number_format($data['valor'], 0, ',', '.') . '</td>';
                            echo '<td style="text-align: center;">$ ' . number_format($data['ingresoRT'], 0, ',', '.') . '</td>';
                            echo '<td style="text-align: center;"><i class="fas fa-check-circle text-info" style="cursor: pointer;" onclick="aprobarComision(' . $psisql . ')" title="Aprobar Comisión"></i></td>';
                            echo '</tr>';
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>¿Está seguro que desea pagar esta comisión? Este proceso no podrá ser revertido.</p>
                    <form id="formConfirmar" action="pays_pays" method="POST">
